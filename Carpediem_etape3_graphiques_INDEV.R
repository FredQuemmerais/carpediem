...
#                 Ils ne peuvent en aucun cas être tenu responsable des interprétations et des conclusions qui pourraient être faites suite à l'utilisation de ce programme.
#
...
library(rpostgis)
library(sp)
#library(RODBC) #ajout 20231017
#library(odbc) #ajout 20231017
library(DBI) #ajout 20231017
library(RPostgres) #ajout 20231017
library(RPostgreSQL)  # pour lire DB et transferer data.frames vers DB PostgreSQL #modif 20231017 #nouvel ajout 13 11 23 lodier
library(dplyr)
#library(postGIStools) # pour charger des tables de la DB PostgreSQL #modif 20231017
#library(rgeos)        
#library(maptools) 
#library(rgdal) #lodier 13 11 23 : package obsol?te
library(sf) # ajout lodier 13 11 23
library(raster)
library(shapefiles)
library(readODS)
library(PBSmapping)
...
#require(RPostgreSQL) #modif 20231017
library(readODS)

#-- pas encore reinstalle sous R.3.4.1
library(grDevices)

#-- a retirer des que les solutions temporaires sont enlevees
library(stringr)
...
#Test debug S4 Lodier - 10 11 23 - FONCTIONNEL !!!!!
con_local <- dbConnect(drv=dbDriver(config$DBConnectlocal$DBConnect_drv),
                       host=config$DBConnectlocal$DBConnect_host,
                       user=config$DBConnectlocal$DBConnect_user,
                       password=config$DBConnectlocal$DBConnect_password,
                       dbname=config$DBConnectlocal$DBConnect_dbname,
...
#Test debug S4 Lodier - 10 11 23
#con_dist <- dbConnect(drv=dbDriver(config$DBConnectdist1$DBConnect_drv),
#                      host=config$DBConnectdist1$DBConnect_host,
#                      user=config$DBConnectdist1$DBConnect_user,
#                      password=config$DBConnectdist1$DBConnect_password,
#                      dbname=config$DBConnectdist1$DBConnect_dbname,
...
for(i in 1:length(codes)){ # Boucle sur toutes les tables
  ...
  #tab <- dbReadSpatial(con, schemaname=config$tabgraph$Data[i_code, i_DB_scheme], tablename=config$tabgraph$Data[i_code, i_DB_table], geomcol="geometry") # maj 26 02 24 lodier # naze
...
activite<- activite %>% arrange(!!sym(IDmesh))
pression<-pression %>% arrange(!!sym(IDmesh))
effet<-effet %>% arrange(!!sym(IDmesh))
index<-index %>% arrange(!!sym(IDmesh))
confiance<-confiance %>% arrange(!!sym(IDmesh))
...
extr_tab <- function(tab, suf){
  ...
  #drop du champ geom?trique de tab (?quivalent de ne travailler qu'avec tab@data)
  tab <- st_drop_geometry(tab)
  i_ID <- which(names(tab) == IDmesh) # modification lodier 14 11 23 -> maj suite ? changement rgdal / sf
  #out <- tab@data[,i_ID] #On initialise la table de sortie avec les identifiants de maille
  out <- tab[, i_ID] # modification lodier 14 11 23 -> maj suite ? changement rgdal / sf
  for(i in 1:dim(tab)[2]){ #Boucle sur toutes les colonnes de la table d'entree
    suf_tab <- substr(colnames(tab)[i], 1, n) #On extrait le debut du nom de la colonne (meme nb de caracteres que le suffixe)
    if(suf_tab==suf){ # Si cela correspond au suffixe
      out <- cbind.data.frame(out, tab[,i]) # On integre la colonne dans le tableau de sortie
      colnames(out)[dim(out)[2]] <- colnames(tab)[i] # On conserve le nom de colonne
    } 
  }
  colnames(out)[1] <- "mesh"
  return(out)
...
### Solution ad-hoc, necessite de retravailler le code -------------------------
...
# Ici au lieu de taper dans A_code, on cherchait les colonnes de activite avec le prefixe decrit par la variable suf_act
# C'est pas pratique et on a plus ça dans l'Etape 1 donc on remet ce suffixe ici
activite <- activite %>% 
  rename_with(
    .fn = ~paste0(suf_act, .),
    .cols = any_of(tolower(config$A_code[,1]))
  )
# Application de la fonction extr_tab
A_zi <- extr_tab(activite, suf_act) 

# Et donc ici on re-enleve suf_act des colonnes de A_zi, parce que si jamais on a besoin des vrais noms des activites plus tard
A_zi <- A_zi %>%
  rename_with(
    .fn = ~str_remove(., paste0("^", suf_act)),   # supprime le prefixe "a_"
    .cols = -mesh                 # toutes les colonnes SAUF "mesh"
  )
...
pression <- pression %>% 
  st_drop_geometry() %>%  
  dplyr::select(any_of(paste0(tolower(config$P_code), "_norm"))) %>% 
  rename_with(~ gsub("_norm$", "", .x)) %>%
  rename_with(
    .fn = ~paste0(suf_pre, .),
    .cols = any_of(tolower(config$P_code))
  )
...
# EVEN MORE NONSENSE
# Comme idmesh est l'ID des SORTIES mais fid_n est l'ID des ENTREES, il faut qu'on renomme le champ fid_n dans zones en idmesh.

zones <- zones %>% rename(idmesh = fid_n)
...
#sub_zones <- zones[,which(names(zones) %in% c(IDmesh, vec_zones))] # MAJ lodier 14 11 23 suite ? transition rgdal -> sf
sub_zones <- zones %>% dplyr::select(which(names(.) %in% c(IDmesh, vec_zones))) # lodier 14 11 23
...
data_index <- st_drop_geometry(index) %>% dplyr::select(which(names(.) %in% c(IDmesh, "ima_option1", "ima_option2", "ipc_option1", "ipc_option2", "refc_e1")))
#data_index <- cbind.data.frame(data_index, confiance@data[,which(colnames(confiance@data) == "ic")])
confiance_no_geom <- st_drop_geometry(confiance) # MAJ lodier 14 11 23
data_index <- cbind.data.frame(data_index, confiance_no_geom[,which(names(confiance_no_geom) == "ic")] ) # MAJ lodier 14 11 23
...
if(length(na.omit(data_index$ic)) == 0){data_index$ic <- 0}
# Ce truc bug si tous les IC sont des NA. Hhhhhhhhhhhh.
...
if("habitats" %in% config$tabgraph$Data[,i_Table_code]){
  ...
  habitats_z <- st_drop_geometry(habitats) # MAJ lodier 14 11 23
...
config$Aiag<-toupper(config$Aiag)
for(i in 2:dim(A_zi)[2]){
...
f_hist_dis <- function(in.tab, in.col, in.title){
  ...
  out <- out +  geom_bar(aes(y = after_stat(count)/sum(after_stat(count))*100), fill="#959ce5") # maj lodier 080324 correction du warning dot-dot notation deprecated ggplot 3.4.0
...
export_graph <- function(in.graph, in.dir, in.namefile){
  if (!file.exists(paste(in.dir, in.namefile, sep="/"))){
    ggsave(in.namefile, plot = in.graph, device = "jpeg" , path = in.dir, width=34, height=38, units=c("cm"))
...
Stat_Ai_plot <- function(tab, lines){
  ...
  for (i in 1:(dim(A_zi)[2]-1)){ # Boucle sur les activites
    ...
    ind.tab <- tibble::rownames_to_column(tab, var = "id")
    Stat[j,3] <- (length(which(A_zi[ind.tab$id, i+1]>0))/nrow(tab))*100
    ...
    Stat[j+1,3] <- (length(which(Contrib_Ai[lines,i+1]>0))/length(lines))*100
    ...
    Stat[j+2,3] <- (sum(Contrib_Ai[lines,i+1])/sum(Contrib_Ai[lines,2:dim(Contrib_Ai)[2]]))*100
    }
  ...
  out <- out + theme(legend.position="bottom", legend.title = element_blank(), legend.text = element_text(size=10), axis.title = element_text(size = 10), axis.text = element_text(size=10))
  return(out)
}
...
# Argument d'entree: les numeros de lignes des mailles sur lesquelles on souhaite travailler
Stat_Pj_plot <- function(tab, lines){
  ...
  for (i in 1:(dim(Mat_Pj_z)[2]-1)){ #Boucle sur les pressions
    ...
    ind.tab <- tibble::rownames_to_column(tab, var = "id")
    Stat[j,3] <- (length(which(Mat_Pj_z[ind.tab$id,i+1]>0))/nrow(tab))*100
    ...
    Stat[j+1,3] <- (length(which(Mat_REF_PjE1_z[lines,icol_REF]>0))/length(lines))*100
    ...
    Stat[j+2,3] <- (sum(Mat_REF_PjE1_z[lines,icol_REF], na.rm=T)/sum(Mat_REF_PjE1_z[lines,2:dim(Mat_REF_PjE1_z)[2]], na.rm = T))*100
    }
...
Stat_Ai_plot_bis <- function(lines){
  ...
  out <- out + theme(axis.title = element_text(size = 10), axis.text = element_text(size=10)) + ylab("Presence (% de mailles)")
  return(out)
}
...
graph_maker_bis <- function(tab, lines, dir){
  ...
  print(length(sub_tab[,4])) # test lodier 071223
  ...
  export_graph(plot1, dir, "Histogramme_IMA1.jpeg")
  # IMA2
  if(max(na.omit(sub_tab[,4])) > 0){ # modification lodier 16 11 23 : ajout de na.omit
    print(max(na.omit(sub_tab[,4])))
    plot2 <- f_hist_cont_bis(na.omit(sub_tab), 4, tab, 4, 5, "IMA2") # modif lodier 07 12 23 na.omit
    export_graph(plot2, dir, "Histogramme_IMA2.jpeg")
  } else {print("Tous les IMA2 sont nuls -> pas d'export du graphe Histogramme_IMA2.jpeg")}  
  # IPC1
  ...
  export_graph(plot3, dir, "Histogramme_IPC1.jpeg")
  # IPC2
  if(max(na.omit(sub_tab[,6])) > 0){
    plot4 <- f_hist_cont_bis(na.omit(sub_tab), 6, tab, 6, 5, "IPC2") # modif lodier 07 12 23 na.omit
    export_graph(plot4, dir, "Histogramme_IPC2.jpeg")
  } else {print("Tous les IPC2 sont nuls -> pas d'export du graphe Histogramme_IPC2.jpeg")}  
...
for(i in 1:1){
  ...
  #field_i <- dplyr::select(sub_zones, which(names(sub_zones)==config$tabgraph$Data[i_zones[i], i_Field])) # MAJ lodier 14 11 23
  field_i<-as.character(field_i) # @Alexis@ MAJ : quand on utilise des numéros de zone, field_i entre un integer dans ggplot(colour = field_i), qui l'interprète comme un n° de couleur et pas comme les noms des zones
  ...
  if(config$tabgraph$Data[i_zones[i], i_Codes[1]] == "BIN"){ 
    ...
    # Cas de figure ou l'on s'interesse a toutes les categories 
      if(config$tabgraph$Data[i_zones[i], i_Codes[1]] == "ALL"){
        ...
        #categories <- subset(field_i, !is.na(field_i)) %>% # MAJ lodier 14 11 23 & 16 11 23
        #              dplyr::distinct(.) %>%
        #  t(.)
      #} 
    ...
    for(j in 1:length(categories)){ # Boucle sur les categories prises en compte
      ...
      lines_j <- as.numeric(which(field_i %in% cat_j)) # extraction des numeros de ligne associes a cette categorie
      #lines_j <- subset(field_i, field_i$b3m_sic_srm == cat_j) %>% # MAJ lodier 14 11 23 
      #  tibble::rownames_to_column(., var = "rnum") %>%
      #    dplyr::select(., rnum) %>%
      #    t(.) %>%
      #    as.numeric(.)
      ...
      #if(nrow(lines_j) > 10) graph_maker_bis(data_index, lines_j, paste(newdir, cat_j, sep="/")) # MAJ lodier 14 11 23
