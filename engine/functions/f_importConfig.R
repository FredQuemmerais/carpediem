# import_param.R                                                      
# Import des fichiers ODS de paramétrisation (matrices et typologies)
...
f_importConfig <- function(path) {
  ...
  #Elements 'Outputs'
  index <- match("export_maps_sensibC_P",res[,1])
  out$Outputs$export_maps_sensibC_P <- f_Int_to_Log(as(as.vector(res[index,2]),res[index,3]))
  index <- match("export_maps_Ai",res[,1])
  out$Outputs$export_maps_Ai <- f_Int_to_Log(as(as.vector(res[index,2]),res[index,3]))
  index <- match("export_maps_PjAi",res[,1])
  out$Outputs$export_maps_PjAi <- f_Int_to_Log(as(as.vector(res[index,2]),res[index,3]))
  index <- match("export_maps_Pj",res[,1])
  out$Outputs$export_maps_Pj <- f_Int_to_Log(as(as.vector(res[index,2]),res[index,3]))
  index <- match("export_maps_REX_Pj",res[,1])
  out$Outputs$export_maps_REX_Pj <- f_Int_to_Log(as(as.vector(res[index,2]),res[index,3]))
  index <- match("export_maps_REF_Pj",res[,1])
  out$Outputs$export_maps_REF_Pj <- f_Int_to_Log(as(as.vector(res[index,2]),res[index,3]))  
  ...
  out$P_code   <- res[2:nlin,which(res[1,]=="Code_P")]  
  ...
  out$P_code_ag   <- res[2:nlin,which(res[1,]=="Code_P_agg_sensib")] ; 
