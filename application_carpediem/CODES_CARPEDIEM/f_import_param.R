...
f_importConfig <- function(path) {
  ...
  res <- as.matrix(read.xlsx(path, sheetName="configuration",encoding="UTF-8",header=FALSE))
  ...
  res <- as.matrix(read.xlsx(path, sheetName="connexion_bd",encoding="UTF-8",header=FALSE))
  #on retire des champs ? lire les espaces et les quotes (ATTENTION : aucun espace dans aucun des champs)
  res[,1:2] <- apply(res[,1:2],2,function(x) gsub(" ","",x))
  res[,1:2] <- apply(res[,1:2],2,function(x) gsub("\"","",x))
  ...
  res  <- as.matrix(read.xlsx(path, sheetName="typologie_pressions",encoding="UTF-8",header=FALSE))
  ...
  res <- as.matrix(read.xlsx(path, sheetName="typologie_activites",encoding="UTF-8",header=FALSE))
  ...
  res <- as.matrix(read.xlsx(path, sheetName="data",encoding="UTF-8",header=FALSE))
  ...
  res <- as.matrix(read.xlsx(path, sheetName="matrice_activites_pressions",encoding="UTF-8",header=FALSE))
  out$Mat_AP <- res
  ...
  res <- as.matrix(read.xlsx(path, sheetName="simulation_monte_carlo",encoding="UTF-8",header=FALSE))
  #on retire des champs ? lire les espaces et les quotes (ATTENTION : aucun espace dans aucun des champs)
  res[,1:2] <- apply(res[,1:2],2,function(x) gsub(" ","",x))
  res[,1:2] <- apply(res[,1:2],2,function(x) gsub("\"","",x))
  ...
  res <- as.matrix(read.xlsx(path, sheetName="statistiques_par_zones",encoding="UTF-8",header=FALSE))
  #reperage du debut des donnees
  indexBEG <- which(res[,1]%in%"*****")[1]; 
  indexEND <- which(res[,1]%in%"*****")[2]; 
  titles <- as.vector(res[(indexBEG+1),]);
...
# -------------------------------------------------------------
# Fonction pour transformer valeurs entieres en logiques
# -------------------------------------------------------------
f_Int_to_Log <- function(in.integer){
  if(in.integer==0){
    out.logical <- FALSE
  }
  if(in.integer==1){
