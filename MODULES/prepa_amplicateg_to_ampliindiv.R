...
fich_amplitudes <- "ampli_par_cat_spp.ods"
ou_mettre_resultat <- "Matrice_multipliee_spp.ods"
...
print(c(paste("Pressions non etudiees :", length(setdiff(rownames(MatAP_bnr), rownames(MatAmpli))) ),
...
MatAmpli <- MatAmpli %>%
  ...
  filter(rowname %in% rownames(MatAP_bnr)) %>%
  column_to_rownames("rowname") %>%
  select(any_of(colnames(MatAP_bnr)))

# Pour eviter tout problème on reduit aussi MatAP_bnr
MatAP_bnr <- MatAP_bnr %>% 
  rownames_to_column("rowname") %>%
  filter(rowname %in% rownames(MatAmpli)) %>%
  column_to_rownames("rowname") %>%
  select(any_of(colnames(MatAmpli)))
...
En_tete <- categorisees$MatAP_depart[1,] %>% 
  ...
  select(Type, Code_A, Activite, Phase, colnames(MatAP_finale))

MatAP_IC <- categorisees$MatAP_depart %>%
  filter(`0...1` == "IC") %>% 
  filter(`0...2` %in% rownames(MatAP_finale)) %>%
  rename(
    Type = `0...1`,
    Code_A = `0...2`,
    Activite = `0...3`,
...
MatAP_finale <- MatAP_finale %>% 
  ...
  left_join(select(MatAP_IC, Type, Code_A, Activite, Phase), by = "Code_A") %>% 
  select(colnames(MatAP_IC)) %>% 
  mutate(Type = "Final") %>% 
  rbind(MatAP_IC) %>% 
  mutate(
