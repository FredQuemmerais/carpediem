...
# Correctrice   : Tiphaine Lodier (2023-2024)
#                 Patch de correction lié à l'obsolescence du package rgdal et de ses dépendances associées
...
# parametrage et dossier utilisateur
fileparam         <- "fichier_parametrage_refc_2024_modele.xlsx"    # fichier de param?trage Excel
userdir           <- "C:/Users/alexis.esquerre/Documents/00_CODE_CARPEDIEM_BENTHOS_2024_KIT_CEREMA/"
...
moddir     <- paste0(userdir,"1_kit_analyses")     
indir      <- paste0(moddir,"/INPUTS") 
outdir     <- paste0(moddir,"/OUTPUTS") 
paramdir   <- paste0(moddir,"/PARAM")
sauvrepdir <- paste0(moddir,"/SAUVREP")
sigdir     <- paste0(indir,"/SIG")
...
library(rpostgis)
library(sp)
library(sf)           # remplacement de rgdal et consort lodier 0224
library(DBI)          # driver db lodier 0224
library(RPostgres)
...
#library(rgeos)     # lodier 0224 dépend de rgdal   
#library(maptools)  # lodier 0224 ca degage
#library(rgdal)     # lodier 0224 goodbye rgdal
library(raster)
library(shapefiles)
library(PBSmapping)
library(readODS)
...
#require(RPostgreSQL) # lodier 0224 ca degage
...
source('f_import_marha_202403.R')                # code source pour importer les parametres / tables de DB
source('f_lec_Postgres_DB_marha_202403.R')       # code source de fonctions de lecture de BD Postgre  
source('./Rpostgis/Rpostgis_marha_202403.R')     # code source Nicolas Lambert - modifie par AVB
source('f_map_plot_marha_202403.R')              # code source fonctions de plot des cartes
# source('f_spatial_data_analysis.R') # Fonctions ouverture couches SIG, ...

## LECTURE DES PARAMETRES DU FICHIER EXCEL ET ECRITURE DANS LA LISTE config ----
file_path <- paste0(paramdir,"/",fileparam)         # path et nom du fichier Excel qui contient les param?tres
...
# - Creation d'une fonction pour convertir le binaire entr? dans le fichier de param?trage en terme de SRM ?tudi?es
# read_srm <- function(x){
#   out <- c()
...
for(i in 1:3){ #On peut renseigner dans le fichier de param?trage jusqu'? 3 seuils diff?rents
  eval(parse(text=paste0("val <- config$simul$thres", i ,collapse=" ; "))) # On va chercher les valeurs configur?es
...
datesauv <- paste0(substr(date(),21,24),substr(date(),5,7),substr(date(),9,10) ) #suffixe du fichier de sauvegarde
...
if(!lrepr_DBtables) {   # Import des tables de la DB si elles n'ont pas ?t? sauvegard?es dans un fichier d'initialisation
  ...
  for(i in 1:ntables){
    ...
    tab_name <- paste0(config$DBData$Data[irow,icolData_type],"_",config$DBData$Data[irow,icolTable_code] )
    ...
    if(config$DBData$Data[irow,which(colnames(config$DBData$Data)=="Source")]=="PgAdmin"){
      ...
      filename <- paste0(sigdir,"/ACTIVITES/",config$DBData$Data[irow,which(colnames(config$DBData$Data)=="DB_table")] )
    ...
    eval(parse(text=paste0(tab_name," <- temp" ,collapse=" ; ")))
    # print(paste('... lecture de la variable ',tab_name, '   ... OK'))
    head(temp)          # lodier 0224
    eval(parse(text=paste0("list_sauv$",tab_name,"<- temp" ,collapse=" ; ")))
  }
  
  # sauvegarde dans un fichier Rdata 
  if(lsauv_DBtables) {
    namedata <- "tables_"
    filesauv <- paste0(sauvrepdir,"/init_",namedata,datesauv,".Rdata" )
  ...
  namedata <- "tables_"
  filerepr <- paste0(sauvrepdir,"/init_",namedata,daterepr,".Rdata" )
  if(!file.exists(filerepr)) stop(paste0("le fichier .Rdata suivant n existe pas : ",filerepr ))
  load(file=filerepr)
  ...
  for(idata in 1:ndata){
    ...
    temp <- eval(parse(text=paste0(iTable_code," <- ","list_sauv$",iTable_code ,collapse=" ; ")))
    # print(paste('... recuperation de la table ',iTable_code, '   ... OK'))
    head(temp)           # lodier 0224
  }
  print(paste0("Reprise de toutes les tables contenues dans le fichier ",filerepr," ... ok" ))
}

## GESTION DES INDICES ET DIMENSIONS, stockage dans des listes ----
# dimensions spatiales, grille
# - lecture et indi?age des mailles de la zone evaluee
grid_z  <- list()
nztot   <- dim(st_coordinates(G_grid))[1] # maj lodier 0224
nz      <- 0 ; iz <- 0;
...
# lodier debug 0224
i_SRM    <- f_idFieldDBTable_from_nameField("srm",st_drop_geometry(config$DBData$Data),G_grid,"grid") # colonne de la grille contenant la SRM de chaque maille
i_surfmer<- f_idFieldDBTable_from_nameField("surfmer",st_drop_geometry(config$DBData$Data),G_grid,"grid") # Colonne de la grille contenant la surface maritime
i_idz    <- f_idFieldDBTable_from_nameField("idmesh",st_drop_geometry(config$DBData$Data),G_grid,"grid") # Colonne de la grille contenant l'identifiant de maille


filesauv_grid_z <- paste0(sauvrepdir,"/init_grid_z_",chainchar,"_",datesauv,".Rdata" )
config$filesauv_grid_z <- filesauv_grid_z

if(config$SauvRep$lrepr_grid_z==0){# le fichier de configuration de la grille n existe pas, lancement de l analyse
  ...
  grid_z <- subset(G_grid, st_drop_geometry(G_grid)[[i_SRM]] %in% lcond_SRM) # lodier 0224
  grid_z <- subset(grid_z, st_drop_geometry(grid_z)[[i_surfmer]] > 0.) #lodier 0224 # commentée par Fred le 2023/12/06 pour test analyse opale pbs surface maille et/ou habitats
  
  nz        <- dim(grid_z)[1]
  
  for(z in 1:nz){
    # print(paste("  - progression analyse de la grille : ",z,"/",nz))
    varsave$long_z[z]    <-  st_coordinates(st_geometry(grid_z))[z,1] # lodier 0224 
    varsave$lat_z[z]     <-  st_coordinates(st_geometry(grid_z))[z,2] # lodier 0224
    varsave$longmax_z[z] <-  max(st_coordinates(st_geometry(grid_z[z,]))[,1]) # debug lodier 0224 
    varsave$longmin_z[z] <-  min(st_coordinates(st_geometry(grid_z[z,]))[,1]) # debug lodier 0224
    varsave$latmax_z[z]  <-  max(st_coordinates(st_geometry(grid_z[z,]))[,2]) # debug lodier 0224
    varsave$latmin_z[z]  <-  min(st_coordinates(st_geometry(grid_z[z,]))[,2]) # debug lodier 0224
  }
  ...
  filerepr_grid_z <- paste0(sauvrepdir,"/init_grid_z_",chainchar,"_",config$SauvRep$daterepr_grid_z,".Rdata" )
  if(!file.exists(filerepr_grid_z)) stop(paste0("le fichier .Rdata suivant n existe pas : ",filerepr_grid_z ))
  print(paste0("# << reprise de la grille sauvegard?e dans le fichier : ",filerepr_grid_z ))
...
# print(paste0("# l'evaluation est r?alis?e sur : ",nz," mailles" ))

config$longmin <- extent(grid_z)[1];  config$longmax <- extent(grid_z)[2]; 
config$latmin <-  extent(grid_z)[3] ; config$latmax <-  extent(grid_z)[4]
# print(paste0("# les limites du domaine ?valu? sont : S:",config$latmin," N:",config$latmax," W:",config$longmin," E:",config$longmax ))
...
col_idz      <- eval(parse(text=paste0("grid_z$",colnames(head(grid_z))[i_idz] ,collapse=" ; ")))
col_surfmer  <- eval(parse(text=paste0("grid_z$","surfmer" ,collapse=" ; ")))
...
# -------------------------------------------------------------------------------------------------
i_surfhab  <- f_idFieldDBTable_from_nameField("surfhab",config$DBData$Data,st_drop_geometry(E_habbenth_sensib),"habbenth_sensib") # lodier 0224
i_idzhab   <- f_idFieldDBTable_from_nameField("idmesh",config$DBData$Data,st_drop_geometry(E_habbenth_sensib),"habbenth_sensib") # lodier 0224
i_surfmesh <- f_idFieldDBTable_from_nameField("surfmesh",config$DBData$Data,st_drop_geometry(E_habbenth_sensib),"habbenth_sensib") # lodier 0224
i_surfmer <- which(colnames(st_drop_geometry(E_habbenth_sensib))=="surfmer")


# - restriction de la table habitats/sensibilit? aux seules mailles de la zone etudi?e
HabBenth_z <- dplyr::filter(E_habbenth_sensib, E_habbenth_sensib[[i_idzhab]] %in% grid_z[[i_idz]]) # MAJ Lodier 0224

# - conversion des surfaces d'habitats m2 -> km2
HabBenth_z[[i_surfhab]] <- HabBenth_z[[i_surfhab]] * conv_m2tokm2 # lodier 0224
...
filesauv_surfhab <- paste0(sauvrepdir,"/init_lHabBenth_z_",chainchar,"_",datesauv,".Rdata" )
...
if(config$SauvRep$lrepr_hab_sensib==0){
  ...
  lHabBenth_z$surfhab_z  <- array(0.,dim=c(nz,1),dimnames=list(paste0("z",1:nz),paste("surfhab")))
  lHabBenth_z$surfmer_z <- array(NA,dim=c(nz,1),dimnames=list(paste0("z",1:nz),paste("surfmer")))
  lHabBenth_z$surfmesh_z <- array(NA,dim=c(nz,1),dimnames=list(paste0("z",1:nz),paste("surfmesh")))
  lHabBenth_z$idmesh_z   <- as.matrix(col_idz) ; dimnames(lHabBenth_z$idmesh_z) <- list(paste0("z",1:nz),paste("mesh"))
  
  for(ipoly in 1:nb_poly){
    # - num?ro de la ligne correspondant ? cet identifiant de maille
    id <- which(lHabBenth_z$idmesh_z == st_drop_geometry(HabBenth_z)[[i_idzhab]][[ipoly]]) # lodier 0224
    # - cumul de surface d'habitat
    lHabBenth_z$surfhab_z[id] <- lHabBenth_z$surfhab_z[id] + st_drop_geometry(HabBenth_z)[[i_surfhab]][[ipoly]] # lodier 0224
    #surface de la maille (en km2)
    lHabBenth_z$surfmesh_z[id] <- st_drop_geometry(HabBenth_z)[[i_surfmesh]][[ipoly]] # lodier 0224
    #surface maritime (en km2)
    lHabBenth_z$surfmer_z[id] <- st_drop_geometry(HabBenth_z)[[i_surfmer]][[ipoly]] # lodier 0224
  }
  
  # - nom des colonnes de la table HabBenth_z
  #lHabBenth_z$P_codes_HabBenth <- colnames(as.matrix(HabBenth_z@data))
  lHabBenth_z$P_codes_HabBenth <- colnames(as.matrix(st_drop_geometry(HabBenth_z))) # lodier 0224
  if(config$SauvRep$lsauv_hab_sensib==1){save(lHabBenth_z,file=filesauv_surfhab)}
} else {
  filerepr_surfhab <- paste0(sauvrepdir,"/init_lHabBenth_z_",chainchar,"_",config$SauvRep$daterepr_hab_sensib,".Rdata" )
  if(!file.exists(filerepr_surfhab)) stop(paste0("le fichier .Rdata suivant n existe pas : ",filerepr_surfhab ))
...
if(config$Calc$l_corr_surfhab){
  ...
  for(ipoly in 1:nb_poly){
    ...
    if(lHabBenth_z$surfhab_z[id] > config$surfmer_z[id]){
      print(paste("# :: surface habitat du polygone corrigee OLD : ",ipoly,HabBenth_z[[i_surfhab]][[ipoly]])) # lodier 0224
      HabBenth_z[[i_surfhab]][[ipoly]] <- HabBenth_z[[i_surfhab]][[ipoly]] * config$surfmer_z[id] / lHabBenth_z$surfhab_z[id] # lodier 0224
      print(paste("#                                         NEW : ",ipoly,HabBenth_z[[i_surfhab]][[ipoly]])) # lodier 0224
      lHabBenth_z$surfhab_z_NEW[id] <- lHabBenth_z$surfhab_z_NEW[id]+ HabBenth_z[[i_surfhab]][[ipoly]] # lodier 0224
    }
  }
...
if(config$simul$lrepr_activite==1){
  filerepr_activite <- paste0(sauvrepdir,"/activite_",chainchar,"_",config$simul$daterepr_activite,".Rdata" )
...
config$A_ilindata <- array(NA,dim=c(ni,1),dimnames=list(paste0("i",1:ni ),c("ilindata")))

# cr?ation du vecteur qui accueille le code activit? agr?g? quand un jeu de donn?es d?crit plusieurs activit?s
config$Aiag <- array(NA,dim=c(ni,1),dimnames=list(paste0("i",1:ni ),c("Aiag")))
...
for(i in 1:ni){ # boucle sur les activit?s d?crites dans le feuillet 'Typo_activit?s'
  # print(paste0("-----------------ACTIVITE ",config$A_code[i], "   //   ",config$A_name[i],"----------------------" ))
  # print(paste("# "))
  ...
  for(icol in 1:nbcol){  # on parcourt les colonnes Code# pour trouver les codes activit?s
    ...
    vec_codesA <- config$DBData$Data[,which(colnames(config$DBData$Data)==paste0("Code",icol ))]
    ...
    if(length(which(as.matrix(vec_codesA) == as.character(config$A_code[i]))) > 0) {   # au moins une correspondance du code activit? trouv?e dans colonne Code#
      ...
      print(paste0("#    :: pas de correspondance dans colonne Code",icol ))
    }
  ...
  if(!is.na(config$A_ilindata)[i]){
    ...
    if(length(vec)>1){
      for(ivec in 2:length(vec)){
        config$Aiag[i] <- paste0(config$Aiag[i],'oui' )
        config$Aiag[i] <- paste0(config$Aiag[i],vec[ivec] )
      }
    }
  }
}
...
if(config$simul$lrepr_activite==0){
  ...
  for(i in 1:ni_ag){
    ...
    Ai_nametab <- paste0("A_",config$DBData$Data[ilin,i_Table_code] )
    
    # r?cup?ration de la table
    Ai_table   <- eval(parse(text=paste0("list_sauv$",Ai_nametab ,collapse=" ; ")))
    
    # print(paste("# -> cette activit? a un jeu de donn?es : ",paste0("list_sauv$",Ai_nametab ,collapse=" ; ")))
    # print(paste("# ->                             champ  : ",config$DBData$Data[ilin,i_Field]))
    
    # - subset de la couche SIG
    Ai_z <- subset(Ai_table, eval(parse(text=paste0("Ai_table$id2 %in% grid_z[[",i_idz,"]]" ,collapse=" ; "))))
    ...
    i_idAi    <- which(colnames(Ai_z)==config$DBData$Data[ilin,i_Field]) # lodier 0224
    ...
    for (i in 1:nz){
      if (vec_zid[i] %in% Ai_z[[which(colnames(Ai_z)=="id2")]]){ # lodier 0224
        vec2 <- subset(st_drop_geometry(Ai_z),st_drop_geometry(Ai_z)[[which(colnames(Ai_z)=="id2")]]==vec_zid[i]) 
...
for(i in 2:dim(A_zi)[2]){
  ...
  colnames(A_zi_lognorm)[i] <- paste0(colnames(A_zi)[i], "_lognorm" )
}
...
if(config$SauvRep$lrepr_iq==0){
  ...
  if(config$factor$X5 == 1){
    ...
    for(i in 1:ni_ag){
      ...
      # nom du champ
      Ai_field <- paste0(config$DBData$Data[ilin,i_Field], "_iq")
      # r?cup?ration de la table
      Ai_table   <- eval(parse(text=paste0("list_sauv$",Ai_nametab ,collapse=" ; ")))
      # - subset de la couche SIG
      Ai_z <- subset(Ai_table, eval(parse(text=paste0("Ai_table$id2 %in% grid_z[[",i_idz,"]]" ,collapse=" ; ")))) # lodier 0224
      ...
      # indice du champ de la table 'Data' rassemblant les donn?es activit?s
      i_idAi    <- which(colnames(st_drop_geometry(Ai_z))==Ai_field) # lodier 0224
      ...
      for (j in 1:nz){
        if (vec_zid[j] %in% Ai_z[[which(colnames(Ai_z)=="id2")]]){ # lodier 0224
          vec2 <- subset(st_drop_geometry(Ai_z),st_drop_geometry(Ai_z)[[which(colnames(Ai_z)=="id2")]]==vec_zid[j]) 
          vec[j] <- mean(as.numeric(unlist(vec2[i_idAi])),na.rm=T)
        }
        
      }
      nameVARz <- paste0(config$Aiag[which(config$Aiag==Codes_i_ag[i])[1]], "_iq")
      VARz <- cbind.data.frame(vec_zid,as.matrix(vec)) ; colnames(VARz) <- c("mesh",nameVARz)
      # -- remplissage de la matrice Intensit? des activit?s A_zi
  ...
  if(config$SauvRep$lsauv_iq==1){
    filesauv_iq <- paste0(sauvrepdir,"/IQ_",chainchar,"_",datesauv,".Rdata" )
    save(A_zi_iq, file = filesauv_iq)
  }
} else {  
  filerepr_iq <- paste0(sauvrepdir,"/IQ_",chainchar,"_",config$SauvRep$daterepr_iq,".Rdata" )
  load(filerepr_iq)
}
...
for(j in 1:dim(MatAP_sub)[2]){
  ...
  if(config$P_modcalc[id_config]==2){
    ...
    for(icol in 1:nbcol){ #Boucle sur les colonnes Code dans le feuillet Data
      vec_codesA <- config$DBData$Data[,which(colnames(config$DBData$Data)==paste0("Code",icol ))] #Colonne du feuillet Data associ?e au num?ro de code icod
      if(length(which(as.matrix(vec_codesA) == as.character(P))) > 0){ #Si la pression est contenue dans la colonne
        ...
        Pj_nametab <- paste0("P_",config$DBData$Data[P_ilindata,i_Table_code] ) 
        Pj_table   <- eval(parse(text=paste0("list_sauv$",Pj_nametab ,collapse=" ; "))) #Sauvegarde des donn?es import?es
        # print(paste("# -> cette pression a un jeu de donn?es : ",paste0("list_sauv$",Pj_nametab ,collapse=" ; ")))
        # print(paste("# ->                             champ  : ",config$DBData$Data[P_ilindata,i_Field]))
        Pj_z <- subset(Pj_table, eval(parse(text=paste0("Pj_table$id2 %in% grid_z[[",i_idz,"]]" ,collapse=" ; ")))) # lodier 0224
        i_idPj    <- which(colnames(Pj_z)==config$DBData$Data[P_ilindata,i_Field]) # lodier 0224
        ...
        for (i in 1:nz){ # Boucle sur toutes les mailles
          ...
          if (vec_zid[i] %in% Pj_z[[which(colnames(Pj_z)=="id2")]]){ # lodier 0224
            vec2 <- subset(st_drop_geometry(Pj_z),st_drop_geometry(Pj_z)[[which(colnames(Pj_z)=="id2")]]==vec_zid[i]) 
...
for(ipoly in 1:nb_poly){ # Boucle sur tous les habitats
  ...
  n_id[ipoly] <- which(lHabBenth_z$idmesh_z==HabBenth_z[[i_idzhab]][[ipoly]]) # lodier 0224
  ratio_hab[ipoly] <- (HabBenth_z[[i_surfhab]][[ipoly]]/HabBenth_z[[i_surfmer]][[ipoly]])
...
antagoniste <- function(vec){
  vec <- as.numeric(unlist(vec)) # test lodier 0224
  n <- length(vec)
  out <- 0
  for (i in 1:n){
...
ID_around_zbis <- function(grille, mat){
  ...
  for(i in 1:n){ #On r?alise la boucle sur toutes les mailles de la matrice
    print(paste0("recherche des mailles adjacentes - maille ",i, "/", n ))
...
Distance <- function(mat, mat_ID, n){
  ...
  for(i in 1:n){
    if(i==1){
      print(paste0("Calcul fonction de distance sur 1 maille") )
      ...
      print(paste0("Calcul fonction de distance sur ", i, " mailles") )
...
printime <- function(time){
  if(time<60) print(paste0("Temps de calcul : ", floor(time), "s" ))
  if(time > 60 && time <3600){
    min <- floor(time/60)
    sec <- time - min*60
    print(paste0("Temps de calcul : ", min, "min ", floor(sec), "s" ))
  }
  if(time>3600){
    hrs <- floor(time/3600)
    min <- floor((time-3600*hrs)/60)
    sec <- time - 3600*hrs - 60*min
    print(paste0("Temps de calcul : ", hrs, "h ", min, "min ", floor(sec), "s" ))
  }
}
...
export_table <- function(in.con, in.scheme, in.grid, in.table, in.nametab){
  print(paste0("Export de la table-----", in.nametab, "-------dans le sch?ma------",in.scheme ))
  ...
  New_Attribut <- merge(index_export,in.table,by.x=name_id,by.y="mesh",all.x=T,sort=FALSE) # lodier 0224
  #index_export@data <- New_Attribut 
  index_export <- New_Attribut # lodier 0224
  dbWriteSpatial(in.con, index_export, schemaname=in.scheme, tablename=in.nametab, replace=T)
  #request <- paste0("select UpdateGeometrySRID('",in.scheme,"', '",in.nametab,"', 'geom', 4326)" )
...
if(config$simul$lsauv_activite==1){
  filesauv_activite <- paste0(sauvrepdir,"/activite_",chainchar,"_",datesauv,".Rdata" )
  sauv_activite <- list()
...
Simul <- function(n){
  ...
  for (k in 1:n){
    ...
    print("----------------------------------------------------------------------------------------")
    print(paste0("DEBUT DE LA SIMULATION N?", k ))
    print("----------------------------------------------------------------------------------------")
    ...
    for(j in 1:nj){
      if(P_evalDEM[j]==1){  #reduction du nombre de pressions pour le d?monstrateur
        print(paste0("Calcul de la sensibilit? ? la pression ",P_code[j]) )
        i_idPcode <- c()
        i_idIC <- c()
        # code de la pression
        P_code_HabBenth <- paste0(tolower(P_code[j]),suf_sensib )
        # identification de la colonne correspondante ? cette pression
        i_idPcode   <- which(colnames(HabBenth_z)==P_code_HabBenth) # lodier 0224
        if(config$factor$X3 == 1){
          # code de l'indice de confiance
          P_code_IC <- paste0(tolower(P_code[j]),"_icas" )
          # identification de la colonne correspondante ? cet indice de confiance
          i_idIC   <- which(colnames(HabBenth_z)==P_code_IC) # lodier 0224
        }
        if(length(i_idPcode)==0){
          print(paste("#  STOP : le code pression",P_code_HabBenth,"n a pas d equivalent dans la table habbenth_sensib :" ))
          stop(paste0(lHabBenth_z$P_codes_HabBenth," / " ))
        }
        
        #cr?ation des vecteurs qui contiennent les r?sultats
        # - score de sensibilit? cumul?
        varName <- paste0("SensibC_",P_code[j] )
        res   <- array(0.,dim=c(nz,1),dimnames=list(paste0("z",1:nz ),varName))
        
        for(ipoly in 1:nb_poly){    # lodier 0224
          if(config$factor$X3 == 1) res[speed_sensib[ipoly,1]] <- res[speed_sensib[ipoly,1]] + speed_sensib[ipoly,2] * f_sensib(HabBenth_z[[i_idPcode]][[ipoly]], HabBenth_z[[i_idIC]][[ipoly]], 5)
          if(config$factor$X3 == 0) res[speed_sensib[ipoly,1]] <- res[speed_sensib[ipoly,1]] + speed_sensib[ipoly,2] * f_sensib2(HabBenth_z[[i_idPcode]][[ipoly]])
        }
    ...
    id_nohab <- which(!Mat_Sensib_Pj[,1] %in% HabBenth_z[[i_idzhab]]) # lodier 0224
    ...
    for(j in 1:dim(MatAP_sub)[2]){
      print(paste0("Cartographie de la pression ",colnames(MatAP_sub)[j], " - ", vec_P_name_eval[j] ))
      P <- colnames(MatAP_sub)[j]
      if(P %in% config$P_code) id_config <- which(config$P_code == P)
      if(P %in% config$P_code_ag) id_config <- which(config$P_code_ag == P)[1]
      Mat_PjAi_z    <- lHabBenth_z$idmesh_z
    ...
    for(j in 2:dim(Mat_Pj_z)[2]){
      ...
      colnames(Mat_Pj_lognorm_z)[j] <- paste0(vec_P_code_eval[j-1],"_lognorm")
    }
    ...
    for(j in 2:dim(Mat_Pj_lognorm_z)[2]){  
      ...
      colnames(Mat_REF_Pj_z)[j] <- paste0("REF_",colnames(Mat_Pj_z)[j] )
    }
    ...
    if(cumul == "additif"){
      ...
      #if(cumul == "antagoniste") REFC[which(refc_sum > 0),2] <- unlist(sapply(unlist(sub), function(x) antagoniste(x[x>0]))) # lodier 0224
      #if(cumul == "synergique") REFC[which(refc_sum > 0),2] <- unlist(sapply(unlist(sub), function(x) synergique(x[x>0])))
      #if(cumul == "antagoniste") REFC[which(refc_sum > 0),2] <- unlist(apply(as.matrix(c(1:dim(sub)[1])), 1, function(x) antagoniste(unlist(sub[x, which(sub[x,] > 0)]))))
      #if(cumul == "synergique") REFC[which(refc_sum > 0),2] <- unlist(apply(as.matrix(c(1:dim(sub)[1])), 1, function(x) synergique(unlist(sub[x, which(sub[x,] > 0)]))))
      # Lodier 13 03 24 - dplyr
      if(cumul == "antagoniste") {
        REFC[which(refc_sum > 0), 2] <- sub %>%
          rowwise() %>%
          mutate(antag = ifelse(any(c_across(starts_with("REF_")) > 0), antagoniste(c_across(starts_with("REF_"))), NA_real_)) %>%
          pull(antag) %>%
          na.omit()
      }
      ...
      if(cumul == "synergique") {
        REFC[which(refc_sum > 0), 2] <- sub %>%
        rowwise() %>%
        mutate(syn = ifelse(any(c_across(starts_with("REF_")) > 0), synergique(c_across(starts_with("REF_"))), NA_real_)) %>%
        pull(syn) %>%
        na.omit()
      }
    }
    ...
    colnames(out)[k+1] <- paste0("simul", k )
    colnames(out)[1] <- "mesh"
...
if(config$simul$export_result==1) { #Seulement si on a param?tr? l'option
  ...
  for(i in 1:length(thres_stat)){
    colnames(simul_stat)[2*i] <- paste0("top", thres_stat[i] ) # Nom des colonnes
    colnames(simul_stat)[(2*i)+1] <- paste0("bottom", thres_stat[i] ) #Nom des colonnes
  }
  for(i in 1:nz){ # Boucle sur toutes les mailles
